...
func init_level(tilemap_layer: TileMapLayer, unit_lookup_callable: Callable) -> void:
	unit_lookup = unit_lookup_callable
	
	grid = AStarGrid2D.new()
	grid.cell_size = Vector2i.ONE * TILE_SIZE
	grid.region = tilemap_layer.get_used_rect()
	grid.update()
	
	for cell in tilemap_layer.get_used_cells():
		if tilemap_layer.get_cell_tile_data(cell).get_custom_data('is_solid'):
